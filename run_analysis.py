...
def main():
    ...
    team_dts = {}
    for d in dt_data:
        if d["image_id"] in team_dts:
            team_dts[d["image_id"]].append(d)
        else:
            team_dts[d["image_id"]] = [d]
    team_split_dts = []
    ...
    print(
        "Loaded [{}] detections from [{}] images.".format(
    ...
    sigmas["basketball"] = (
        ...
        / 10.0
    )  # sigma in OKS for keypoint detection
    sigmas["coco"] = None
    ...
    coco_analyze = COCOanalyze(
        coco_gt, coco_dt, "keypoints", sigmas[dataset], dataset=dataset
    )
    ...
    ## regular evaluation
    coco_analyze.evaluate(
        verbose=True, makeplots=True, savedir=saveDir, team_name=teamName
    )
