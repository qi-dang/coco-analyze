...
def backgroundFalseNegErrors(coco_analyze, imgs_info, saveDir):
    ...
    badFalseNeg = coco_analyze.false_neg_gts["all", str(0.5)]
    ...
    for tind, t in enumerate(show_fn):
        ...
        f.write(
            "Image_id, ground_truth id, num_keypoints: [%d][%d][%d]\n"
    ...
    for i, b in enumerate(fn_gts):
        ...
        if b_width < 1024 and b_height < 1024:
            col = [
                ...
                if ar_bins[i] < b_width < ar_bins[i + 1]
            ]
            row = [
                i
                for i in range(len(ar_bins) - 1)
                if ar_bins[i] < b_height < ar_bins[i + 1]
            ]
            ar_pic_2[row, col] += 1

            col = [
                i
                for i in range(len(ar_bins_3) - 1)
                if ar_bins_3[i] < b_width < ar_bins_3[i + 1]
            ]
            row = [
                i
                for i in range(len(ar_bins_3) - 1)
                if ar_bins_3[i] < b_height < ar_bins_3[i + 1]
            ]
            ar_pic_3[row, col] += 1
        else:
        ...
        anns = coco_analyze.cocoGt.loadAnns(
            coco_analyze.cocoGt.getAnnIds(b["image_id"])
        )
        ...
        for poly_verts in b["segmentation"]:
            path = mplPath.Path(
                np.array([[x, y] for x, y in zip(poly_verts[0::2], poly_verts[1::2])])
            )
    ...
    plt.xticks(
        range(1, len(ar_bins) + 1), ["%d" % (x) for x in ar_bins], rotation="vertical"
    )
    ...
    plt.xticks([-0.5 + x for x in range(11)], ["%d" % (x) for x in ar_bins_3])
    plt.yticks([-0.5 + x for x in range(11)], ["%d" % (x) for x in ar_bins_3])
    plt.colorbar()
    plt.grid()
    plt.title("BBox Aspect Ratio", fontsize=20)
    plt.xlabel("Width (px)", fontsize=20)
    ...
    plt.bar(
        range(5), [small, medium, large, xlarge, xxlarge], color="g", align="center"
    )
    ...
    plt.bar(
        range(5),
        [no_people, one, small_grp, large_grp, crowd],
        color="g",
        align="center",
    )
    ...
    plt.figure(figsize=(10, 10))
    plt.bar(
        [k for k in num_fn_keypoints.keys() if k != 0],
        [num_fn_keypoints[k] for k in num_fn_keypoints.keys() if k != 0],
        align="center",
    )
