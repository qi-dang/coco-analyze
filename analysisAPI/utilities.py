...
def show_dets(coco_dts, coco_gts, img_info, save_path=None):
    ...
    for ann in coco_gts:
        ...
        if "keypoints" in ann and type(ann["keypoints"]) == list:
            ...
            for x1, y1, sigma1 in zip(x[v > 0], y[v > 0], sigmas[v > 0]):
                ...
                circle = plt.Circle(
                    (x1, y1), sqrt_neg_log_oks[0] * r, fc=(1, 0, 0, 0.4), ec="k"
                )
                ax.add_patch(circle)
    ...
    for ann in coco_dts:
        ...
        if len(coco_dts) == 1:
            ...
            if len(coco_gts) == 1:
                oks = compute_kpts_oks(
                    coco_dts[0]["keypoints"],
                    coco_gts[0]["keypoints"],
                    coco_gts[0]["area"],
                )
                title = "[%.3f][%.3f][%d][%d][%d]" % (
                    score,
