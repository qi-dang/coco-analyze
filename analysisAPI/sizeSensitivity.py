...
def sizeSensitivity(coco_analyze, oks, saveDir):
    ...
    x = [1, 2, 3, 4]
    y = [
        len(size_index["medium"]),
        len(size_index["large"]),
        len(size_index["xlarge"]),
        len(size_index["xxlarge"]),
    ]
    ...
    for s in stats:
        if s["err"] != "false_neg":
            ...
            bckgd_fp = [
                ss
                for ss in stats
                if (ss["err"], ss["areaRngLbl"]) == ("bckgd_false_pos", s["areaRngLbl"])
            ][0]
    ...
    for err in err_types:
        if err == "false_neg":
            ...
            size_performance[err] = [
                err_perf[err, area] - err_perf["baseline", area] for area in areaRngLbls
            ]
    ...
    plt.scatter(x, y, c="b", s=150, alpha=0.5, edgecolor="black", linewidth=2)
    plt.plot([0.5, 2], [perf_jitt[4], perf_jitt[4]], "r--", linewidth=2)
    ...
    x = [1, 2, 3, 4]
    y = oks_75_auc[:4]
    plt.bar(x, y, color="b", alpha=0.7, align="center", width=0.85)
    plt.plot([0.5, 4.5], [oks_75_auc[4], oks_75_auc[4]], "r--", linewidth=3)
    ...
    f.write(
        ...
        % (
            oks,
            max(oks_75_auc[:4]) - min(oks_75_auc[:4]),
            max(oks_75_auc[:4]) - oks_75_auc[4],
        )
    )
    f.write(
        "Jitter:    Sensitivity[%.3f], Impact[%.3f]\n"
        % (max(perf_jitt[:4]) - min(perf_jitt[:4]), max(perf_jitt[:4]) - perf_jitt[4])
    )
    f.write(
        "Inversion: Sensitivity[%.3f], Impact[%.3f]\n"
        % (max(perf_inv[:4]) - min(perf_inv[:4]), max(perf_inv[:4]) - perf_inv[4])
    )
    f.write(
        "Swap:      Sensitivity[%.3f], Impact[%.3f]\n"
        % (max(perf_swap[:4]) - min(perf_swap[:4]), max(perf_swap[:4]) - perf_swap[4])
    )
    f.write(
        "Miss:      Sensitivity[%.3f], Impact[%.3f]\n"
        % (max(perf_miss[:4]) - min(perf_miss[:4]), max(perf_miss[:4]) - perf_miss[4])
    )
    f.write(
        "Score:     Sensitivity[%.3f], Impact[%.3f]\n"
        % (
            max(perf_score[:4]) - min(perf_score[:4]),
            max(perf_score[:4]) - perf_score[4],
        )
    )
    f.write(
        "Bkgd FP:   Sensitivity[%.3f], Impact[%.3f]\n"
        % (
            max(perf_bk_fp[:4]) - min(perf_bk_fp[:4]),
            max(perf_bk_fp[:4]) - perf_bk_fp[4],
        )
    )
    f.write(
        "FN:        Sensitivity[%.3f], Impact[%.3f]\n"
        % (
            max(perf_bk_fn[:4]) - min(perf_bk_fn[:4]),
            max(perf_bk_fn[:4]) - perf_bk_fn[4],
        )
    )
