...
def localizationErrors(coco_analyze, imgs_info, saveDir):
    ...
    dt_gt_matches = coco_analyze.localization_matches[
        "all", str(coco_analyze.params.oksLocThrs), "dts"
    ]
    ...
    f.write(
        "{:30} [{}]-[{}]\n".format(
            " - Good,      [tot]-[perc]:", int(good), 100 * (good / tot)
        )
    )
    f.write(
        "{:30} [{}]-[{}]\n".format(
            " - Jitter,    [tot]-[perc]:", int(jitter), 100 * (jitter / tot)
        )
    )
    f.write(
        "{:30} [{}]-[{}]\n".format(
            " - Inversion, [tot]-[perc]:", int(inversion), 100 * (inversion / tot)
        )
    )
    f.write(
        "{:30} [{}]-[{}]\n".format(
            " - Swap,      [tot]-[perc]:", int(swap), 100 * (swap / tot)
        )
    )
    f.write(
        "{:30} [{}]-[{}]\n\n".format(
            " - Miss,      [tot]-[perc]:", int(miss), 100 * (miss / tot)
        )
    )
    ...
    rects_d = {}
    rects_d["nose"] = 0.47, 0.75, 0.07, 0.07
    rects_d["left_eye"] = 0.5, 0.83, 0.07, 0.07
    rects_d["right_eye"] = 0.44, 0.83, 0.07, 0.07
    rects_d["left_ear"] = 0.54, 0.77, 0.07, 0.07
    rects_d["right_ear"] = 0.4, 0.77, 0.07, 0.07
    rects_d["left_shoulder"] = 0.58, 0.68, 0.1, 0.1
    rects_d["right_shoulder"] = 0.32, 0.65, 0.1, 0.1
    rects_d["left_elbow"] = 0.67, 0.6, 0.1, 0.1
    rects_d["right_elbow"] = 0.27, 0.52, 0.1, 0.1
    rects_d["left_wrist"] = 0.59, 0.49, 0.1, 0.1
    rects_d["right_wrist"] = 0.34, 0.42, 0.1, 0.1
    rects_d["left_hip"] = 0.48, 0.5, 0.1, 0.1
    rects_d["right_hip"] = 0.39, 0.5, 0.1, 0.1
    rects_d["left_knee"] = 0.55, 0.32, 0.1, 0.1
    rects_d["right_knee"] = 0.4, 0.32, 0.1, 0.1
    rects_d["left_ankle"] = 0.55, 0.15, 0.1, 0.1
    rects_d["right_ankle"] = 0.4, 0.15, 0.1, 0.1
    order = [
        "nose",
        "left_eye",
        "right_eye",
    ...
    for j, err_type in enumerate(["Jitter", "Inversion", "Swap", "Miss"]):
        ...
        rect = -0.03, 0, 0.45, 0.9
        ax1 = fig.add_axes(rect)
        colors = [
            c.rgb
            for c in list(Color("white").range_to(Color(COLORS[j]), len(KEYPOINTS_L)))
        ]
        ...
        for tind, t in enumerate(top_err_dts[0:7]):
            ...
            for sk in sks:
                if USE_VISIBILITY_FOR_PLOTS and v[sk[0]] * v[sk[1]] == 0:
                    ...
                    plt.plot(
                        x[sk], y[sk], linewidth=3, color=utilities.colors[sk[0], sk[1]]
                    )
