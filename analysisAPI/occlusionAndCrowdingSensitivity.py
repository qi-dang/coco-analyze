...
def occlusionAndCrowdingSensitivity(coco_analyze, oks, saveDir):
    ...
    arearangelabel = "all"
    maxdets = coco_analyze.params.maxDets[0]
    ...
    for i in benchmark_overlap.keys():
        for j in benchmark_keypoint.keys():
            ...
            benchmark[i, j]["gtIds"] = list(
                set(benchmark_overlap[i]["gtIds"]) & set(benchmark_keypoint[j]["gtIds"])
            )
    ...
    for e in err_types:
        err_mat[e] = np.zeros((3, 4))
    ground_truth_num_keypoints = np.zeros((3, 4))
    for no, nk in sorted(
        benchmark.keys(), key=lambda element: (element[0], element[1])
    ):
        f.write("\nBenchmark Index [%d,%d]:\n" % (no, nk))
        ...
        coco_analyze.summarize(
            makeplots=True, savedir=loc_dir, team_name="%d-%d" % (no, nk)
        )
        paths[
            "occlusion_crowding_%d%d" % (no, nk)
        ] = "%s/error_prc_[%d-%d][%d][%s][%d].pdf" % (
            loc_dir,
            no,
            nk,
            int(oks * 100),
        ...
        for gtId in benchmark[no, nk]["gtIds"]:
            ground_truth_num_keypoints[no, nk] += coco_gt.loadAnns(gtId)[0][
                "num_keypoints"
            ]

        # analyze sensitivity of localization errors
        corrected_dts = coco_analyze.corrected_dts["all"]
        matches = coco_analyze.localization_matches["all", str(0.1), "gts"]
        matched_gts = [
            matches[g] for g in matches if int(g) in benchmark[no, nk]["gtIds"]
        ]
    ...
    for eind, e in enumerate(err_types[1:]):
        ...
        ax.set_aspect(1)
        res = ax.imshow(
            err_mat[e] / total_keypoints, cmap=cmaps[eind], interpolation="nearest"
        )
        ...
        for x in range(width):
            for y in range(height):
                ax.annotate(
                    "%.1f" % (100 * err_mat[e][x, y] / float(total_keypoints[x, y])),
                    xy=(y, x),
                    horizontalalignment="center",
                    verticalalignment="center",
                    fontsize=20,
