...
def errorsAPImpact(coco_analyze, saveDir):
    ...
    oksThrs = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    ...
    # err_types   = ['miss','swap', 'inversion', 'jitter', 'score', 'false_neg']
    err_colors = [
        "#F2E394",
        "#F2AE72",
        "#D96459",
    ...
    for eind, err in enumerate(err_types):
        ...
        coco_analyze.summarize(
            makeplots=True, savedir=loc_dir + "/all_plots", team_name=err
        )
    ...
    for eind, err in enumerate(err_types):
        ...
        for aind, areaRngLbl in enumerate(areaRngLbls):
            res = sorted(
                [x for x in stats if x["err"] == err and x["areaRngLbl"] == areaRngLbl],
                key=lambda k: k["oks"],
            )
            if err != "false_neg":
                error_perf[err, areaRngLbl] = [
                    x["auc"] - baseline_perf[x["oks"], x["areaRngLbl"]] for x in res
                ]
            else:
                bfp_res = sorted(
                    [
                        x
                        for x in stats
                        if x["err"] == "bckgd_false_pos"
                        and x["areaRngLbl"] == areaRngLbl
                    ],
                    key=lambda k: k["oks"],
                )
                error_perf[err, areaRngLbl] = [
                    x["auc"] - k["auc"] for (x, k) in zip(res, bfp_res)
                ]
    ...
    for aind, areaRngLbl in enumerate(areaRngLbls):
        ...
        for eind, err in enumerate(err_types):
            ...
            errs = error_perf[err, areaRngLbl]
            means.append(np.mean(errs))
            stds.append(np.std(errs))
            colors.append(err_colors[eind])

            rects1 = ax.bar(oksThrs, errs, 0.04, color=err_colors[eind], align="center")
            ax.set_xticks(oksThrs)

            plt.title(
                "Error Type: [%s], Instances Size: [%s]" % (err, areaRngLbl),
                fontsize=20,
            )
            ...
            plt.grid()
            plt.savefig(
                "%s/%s_[%s].pdf" % (loc_dir, err.title(), areaRngLbl),
                bbox_inches="tight",
            )
            plt.close()
    ...
    for aind, areaRngLbl in enumerate(areaRngLbls):
        ...
        rects = ax.bar(
            xs[indx_start:indx_end],
            means[indx_start:indx_end],
            1,
            color=colors[indx_start:indx_end],
            yerr=stds[indx_start:indx_end],
        )
        lgd = fig.legend(
            rects[: len(err_types)],
            err_types,
        ...
        ax.set_xticks([xticks[aind]])
        ax.set_xticklabels([areaRngLbls[aind]], fontsize=20)
        plt.ylabel("AP Improvement", fontsize=20)
        plt.xlabel("Instances Size", fontsize=20)
        path = "%s/ap_improv_areas_%s.pdf" % (loc_dir, areaRngLbl)
        if areaRngLbl == "all":
            paths["ap_improv_areas"] = path
        plt.grid()
    ...
    ax.set_xticklabels(areaRngLbls, fontsize=20)
    plt.ylabel("AP Improvement", fontsize=20)
    plt.xlabel("Instances Size", fontsize=20)
    plt.grid()
    plt.savefig("%s/ap_improv_areas_overall.pdf" % loc_dir, bbox_inches="tight")
    plt.close()
    ...
    for eind, err in enumerate(err_types):
        errs = [
            np.mean(
                [
                    error_perf[err, "all"][i],
                    error_perf[err, "medium"][i],
                    error_perf[err, "large"][i],
                ]
            )
            for i in range(len(oksThrs))
        ]
        stds = [
            np.std(
                [
        ...
        ax.fill_between(
            oksThrs, y1, y2, facecolor=err_colors[eind], alpha=0.6, interpolate=True
        )

    ax.set_xticks(oksThrs), plt.xlim([oksThrs[0] - 0.05, oksThrs[-1] + 0.05])
    plt.ylabel("AP Improvement", fontsize=20)
    plt.xlabel("Oks Threshold", fontsize=20)
    # plt.title('Instances Size: [all]',fontsize=20)
    paths["ap_improv_oks"] = "%s/ap_improv_oks.pdf" % loc_dir
    plt.grid()
    plt.savefig(paths["ap_improv_oks"], bbox_inches="tight")
    plt.close()
