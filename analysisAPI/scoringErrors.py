...
def scoringErrors(coco_analyze, oks, imgs_info, saveDir):
    ...
    paths["opt_score_prc"] = "%s/error_prc_[scoring][%d][%s][%d].pdf" % (
        loc_dir,
        int(oks * 100),
    ...
    for imgId in imgs_info:
        if imgId in all_dts:
            ...
            all_dts[imgId]["score"] = np.argsort(
                [-d["score"] for d in dts], kind="mergesort"
            )
        ...
        for g in gts:
            ...
            if g["ignore"] or (
                g["area"] < coco_analyze.params.areaRng[0][0]
                or g["area"] > coco_analyze.params.areaRng[0][1]
            ):
                g["_ignore"] = 1
            else:
                g["_ignore"] = 0
    ...
    for score_type in scoring_errors.keys():
        for ind, imgId in enumerate(all_dts.keys()):
            ...
            for gind, gt in enumerate(all_gts[imgId]):
                ...
                dts_available = [
                    (i, sorted_dts[i]["id"], oks[i], sorted_dts[i][score_type])
                    for i in dts_with_oks
                    if sorted_dts[i]["id"] not in used_dts
                ]
                if len(dts_available) == 0:
                    break
    ...
    ori_scoring_errors.sort(
        key=lambda k: -np.sqrt(
            (k["matched_dt"]["score"] - k["top_match_dt"]["score"])
            * (k["high_oks"] - k["low_oks"])
        )
    )
    for ind, err in enumerate(ori_scoring_errors[0:12]):
        ...
        f.write("- gt id: [%d]\n" % err["gt"]["id"])
        f.write(
            "- dt id, high score, low  oks:  [%d][%.3f][%.3f]\n"
            % (err["matched_dt"]["id"], err["matched_dt"]["score"], err["low_oks"])
        )
        f.write(
            "- dt id, low  score, high oks:  [%d][%.3f][%.3f]\n"
    ...
    for ind, imgId in enumerate(all_dts.keys()):
        ...
        for gind, gt in enumerate(all_gts[imgId]):
            ...
            dts_high_oks_i = np.where(dts_oks > 0.1)[0]
            ...
            if len(dts_high_oks_i) >= 2:
                ...
                oks_vals = sorted(
                    [(dts_oks[i], dts[i]) for i in dts_high_oks_i], key=lambda k: -k[0]
                )
    ...
    plt.hist(
        num_dts_high_oks,
        bins=[i - 0.5 for i in range(max(num_dts_high_oks) + 1)],
        color="green",
    )
    plt.grid()
    ...
    width = (max_val - min_val) / 20.0
    ax.bar(
        np.linspace(min_val, max_val, 19),
        overlapbins,
        color="red",
        alpha=0.65,
        width=width,
    ...
    plt.grid()
    plt.xlim(
        [min_val - (max_val - min_val) / 20.0, max_val + (max_val - min_val) / 20.0]
    )
