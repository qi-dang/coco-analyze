...
def backgroundFalsePosErrors(coco_analyze, imgs_info, saveDir):
    ...
    for tind, t in enumerate(show_fp):
        ...
        f.write(
            "Image_id, detection_id, score: [%d][%d][%.3f]\n"
    ...
    f.write(
        ...
        % (
            p_20,
            len([d for d in coco_analyze.corrected_dts["all"] if d["score"] <= p_20]),
        )
    )
    f.write(
        ...
        % (
            p_40,
            len([d for d in coco_analyze.corrected_dts["all"] if d["score"] <= p_40]),
        )
    )
    f.write(
        ...
        % (
            p_60,
            len([d for d in coco_analyze.corrected_dts["all"] if d["score"] <= p_60]),
        )
    )
    f.write(
        ...
        % (
            p_80,
            len([d for d in coco_analyze.corrected_dts["all"] if d["score"] <= p_80]),
        )
    )
