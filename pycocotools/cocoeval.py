...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType="segm", sigmas=None):
        """
        ...
        if not iouType:
            print(
                "<{}:{}>iouType not specified. use default iouType segm".format(
    ...
    def evaluate(self, check_scores=False):
        ...
        if check_scores:
            print(
                "<{}:{}>Running per image *optimal score* evaluation...".format(
                    __author__, __version__
                )
            )
        else:
            print(
                "<{}:{}>Running per image evaluation...".format(__author__, __version__)
            )
        ...
        if not p.useSegm is None:
            ...
            print(
                "<{}:{}>useSegm (deprecated) is not None. Running {} evaluation".format(
                    __author__, __version__, p.iouType
                )
            )
        ...
        if check_scores and p.iouType != "keypoints":
            raise Exception(
                "<{}:{}>This function works only for *keypoints* eval.".format(
                    __author__, __version__
                )
            )
        ...
        maxDet = p.maxDets[-1]
        self.evalImgs = [
            evaluateImg(imgId, catId, areaRng, maxDet, check_scores)
            for catId in catIds
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet, check_scores):
        ...
        dtMatchesMax = []
        gtMatchesMax = []
        dtIousMax = [0.0 for d in dt] if check_scores else []
        gtIousMax = [0.0 for g in gt] if check_scores else []
    ...
    def accumulate(self, p=None):
        ...
        """
        print(
            "<{}:{}>Accumulating evaluation results...".format(__author__, __version__)
        )
        tic = time.time()
    ...
    def summarize(self, verbose=False):
        ...
        def _summarizeKps_verbose():
            ...
            mainAreaRng = (
                "all" if len(self.params.areaRngLbl) > 1 else self.params.areaRngLbl[0]
            )

            for t in self.params.iouThrs:
                cur_stat += 1
                stats[cur_stat] = _summarize(
                    1, maxDets=20, iouThr=t, areaRng=mainAreaRng
                )
            ...
            for t in self.params.iouThrs:
                cur_stat += 1
                stats[cur_stat] = _summarize(
                    0, maxDets=20, iouThr=t, areaRng=mainAreaRng
                )
        ...
        if not self.eval:
            raise Exception(
                "<{}:{}>Please run accumulate() first".format(__author__, __version__)
            )
