...
class COCOanalyze:
    ...
    def __init__(
        self, cocoGt, cocoDt, iouType="keypoints", sigmas=None, dataset="soccer"
    ):
        """
        ...
        self.params.teamMaxDets = [
            max([len(self.cocoEval._dts[k]) for k in self.cocoEval._dts.keys()])
        ]
    ...
    def find_keypoint_errors(self):
        ...
        for aind, arearnglbl in enumerate(areaRngLbl):
            self.localization_matches[
                arearnglbl, str(self.params.oksLocThrs), "dts"
            ] = dtMatches[arearnglbl, str(self.params.oksLocThrs)]
            self.localization_matches[
                arearnglbl, str(self.params.oksLocThrs), "gts"
    ...
    def _find_dt_gt_matches(self, oksThrs, areaRng, areaRngLbl):
        ...
        for aind, arearnglbl in enumerate(areaRngLbl):
            ...
            evalImgs = [
                e
                for e in filter(None, self.cocoEval.evalImgs)
            ...
            for did in localization_matches_dts:
                ...
                for a in image_anns:
                    ...
                    inv_keypoints = np.insert(
                        ys[self.params.inv_idx].astype(np.float),
                        np.arange(self.params.num_kpts),
                ...
                miss_kpts = (
                    np.logical_and(oks_max < self.params.jitterKsThrs[0], gt_kpt_v != 0)
                    * 1
                )
                ...
                round_oks_max = (
                    oks_max
                    + np.spacing(1) * (oks_max == 0)
                    - np.spacing(1) * (oks_max == 1)
                )

                dist_to_oks_max = np.sqrt(
                    -np.log(round_oks_max) * 2 * gt["area"] * (self.params.sigmas**2)
                )
        ...
        for areaRng, areaRngLbl in zip(self.params.areaRng, self.params.areaRngLbl):
            ...
            for oind, oks in enumerate(oksThrs):
                ...
                for d in dtMatchesAreaOks:
                    ...
                    assert d in [
                        dt["dtId"]
                        for dt in gtMatchesAreaOks[dtMatchesAreaOks[d][0]["gtId"]]
                    ]
                ...
                false_pos = set(
                    [dt["id"] for dt in self._dts if dt["id"] not in dtMatchesAreaOks]
                )
                ...
                false_neg = set(
                    [gt for gt in self.cocoGt.getAnnIds() if gt not in gtMatchesAreaOks]
                )
    ...
    def summarize(self, makeplots=False, savedir=None, team_name=None):
        ...
        if not self.corrected_dts:
            raise Exception(
                "<{}:{}>Please run analyze() first".format(__author__, __version__)
            )
    ...
    def _summarize_kpt_errors(self):
        ...
        for aind, arearnglbl in enumerate(self.params.areaRngLbl):
            ...
            for eind, err in enumerate(err_types):
                    ...
                    if sum(cdt[err]) != 0:
                        ...
                        for d in self.cocoEval._dts[image_id, self.params.catIds[0]]:
                            if d["id"] == dtid:
                                ...
                                all_kpts = np.delete(
                                    np.array(d["keypoints"]), slice(2, None, 3)
                                )
                                opt_kpts = np.delete(
                                    np.array(corrected_kpts), slice(2, None, 3)
                                )

                                kpts = (
                                    all_kpts * oth_kpts_mask + opt_kpts * err_kpts_mask
                                )
                ...
                ps_mat_kpts[tind_start:tind_end, :, :, aind, :] = self.cocoEval.eval[
                    "precision"
                ][::-1, :, :, 0, :]
                rs_mat_kpts[tind_start:tind_end, :, aind, :] = self.cocoEval.eval[
                    "recall"
                ][::-1, :, 0, :]
    ...
    def _summarize_score_errors(self):
        ...
        for aind, arearnglbl in enumerate(self.params.areaRngLbl):
            ...
            ps_mat_score[:, :, :, aind, :] = self.cocoEval.eval["precision"][
                ::-1, :, :, 0, :
            ]
    ...
    def _summarize_bckgd_errors(self):
        ...
        for aind, arearnglbl in enumerate(self.params.areaRngLbl):
            ...
            for oind, oks in enumerate(oksThrs):
                ...
                self.cocoEval.accumulate()
            ps_mat_false_pos[:, :, :, aind, :] = self.cocoEval.eval["precision"][
                ::-1, :, :, 0, :
            ]
            rs_mat_false_pos[:, :, aind, :] = self.cocoEval.eval["recall"][
                ::-1, :, 0, :
            ]
            ...
            for oind, oks in enumerate(oksThrs):
                ...
                # accumulate results after having set all this ignores
                self.cocoEval.accumulate()
                ps_mat_false_neg[oind, :, :, aind, :] = self.cocoEval.eval["precision"][
                    oind, :, :, 0, :
                ]
                rs_mat_false_neg[oind, :, aind, :] = self.cocoEval.eval["recall"][
                    oind, :, 0, :
                ]
            ...
            for oind, oks in enumerate(oksThrs):
                for aind, arearng in enumerate(areaRngLbl):
                    for mind, maxdts in enumerate(maxDets):
                        ...
                        stat["recall"] = (
                            -1 if len(r[r > -1]) == 0 else np.mean(r[r > -1])
                        )
                        stats.append(stat)
        return stats
    ...
    def _plot(
        ...
        if err_labels:
            ...
            colors[-len(err_labels) :] = [Color(c) for c in color_vec]
        else:
            ...
            for mind, m in enumerate(maxDets):
                ...
                for tind, t in enumerate(iouThrs):
                    ...
                    if err_labels:
                        ...
                        ax = fig.add_axes([0.1, 0.15, 0.56, 0.7])
                        plt.title(
                            "oksThrs:[{}], areaRng:[{}], maxDets:[{}]".format(t, a, m),
                            fontsize=18,
                        )
                        thresh_idx = [
                            tind + i * len(iouThrs) for i in range(len(labels))
                        ]
...
class Params:
    ...
    def setKpParams(self, sigmas=None, dataset="soccer"):
        ...
        self.inv_idx = [
            self.inv_kpts_name.index(self.kpts_name[i]) for i in range(self.num_kpts)
        ]
        if sigmas is None:
            # coco sigmas
